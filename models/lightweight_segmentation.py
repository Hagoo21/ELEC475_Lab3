...
OPTIMIZED VERSION:
- Reduced ASPP channels (256→128) for efficiency
- 3 ASPP branches instead of 4
- Depthwise separable convolutions in decoder
- Multi-level skip connections (low + mid) for better mIoU
- Target: <3M parameters while maintaining 58%+ mIoU
...
class DepthwiseSeparableConv(nn.Module):
    ...
    This reduces parameters significantly while maintaining performance.
    """
...
class ASPPModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=128, dilation_rates=[1, 6, 12]):
        ...
        # Branches 2-3: 3x3 atrous convolutions with different dilation rates
...
class LightweightSegmentationModel(nn.Module):
    ...
    - Context: ASPP module with dilation rates {1, 6, 12} and 128 channels
    - Decoder: Multi-level skip connections (low + mid) + depthwise separable convs
    ...
    def __init__(self, num_classes=21, pretrained=True, return_features=False):
        ...
        # OPTIMIZED: Reduced channel projections
        # Low-level feature projection (stride 4, 16 channels → 32)
        ...
        # Mid-level feature projection (stride 8, 24 channels → 32)
        ...
        # ASPP on high-level features (stride 16, 576 channels → 128)
        self.aspp = ASPPModule(in_channels=576, out_channels=128, 
                              dilation_rates=[1, 6, 12])
        
        # OPTIMIZED: Depthwise separable convolutions in decoder
        # Input: 128 (ASPP) + 32 (mid-level) = 160 channels
        # We'll first upsample ASPP to stride 8, concat with mid-level, then to stride 4
        self.decoder_stage1 = DepthwiseSeparableConv(128 + 32, 128, kernel_size=3, padding=1)
        
        # After upsampling to stride 4, concat with low-level: 128 + 32 = 160
        self.decoder_stage2 = DepthwiseSeparableConv(128 + 32, 128, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        # Project and concatenate with mid-level features
